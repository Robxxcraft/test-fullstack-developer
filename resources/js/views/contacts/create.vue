    ...
    <div class="px-md-5 pt-md-3 min-vh-100 d-flex flex-column">
        ...
        <div class="my-5 card w-50 mx-auto border-0 shadow-sm">
            <div class="card-body bg-white">
                <div class="ps-3 fw-bold text-primary fs-5 text-darkgray">Add Contact</div>
                <form @submit.prevent="saveContact" class="p-3">
                    <div class="pb-3 position-relative">
                        <label class="form-label"><span class="text-danger">*</span> Name</label>
                        <input @focus="clearErrors('name')" type="text" name="name" v-model="form.name" :class="`form-control mb-1 ${ errors.name ? 'is-invalid' : ''}`" placeholder="Enter name">
                        <div v-if="errors.name" class="invalid-feedback position-absolute bottom-0 ps-2">{{ errors.name[0] }}</div>
                    </div>
                    <div class="pb-3 position-relative">
                        <label class="form-label"><span class="text-danger">*</span> Email</label>
                        <input @focus="clearErrors('email')" type="text" name="email" v-model="form.email" :class="`form-control mb-1 ${ errors.email ? 'is-invalid' : ''}`" placeholder="Enter email">
...
export default {
    ...
    setup() {
        ...
        const saveContact = () => {
            loading.value = true
            axios.post('/api/contacts/create', form).then(res=>{
                loading.value = false
                toastStore.mutations.setToast(res.data.success)
                router.push({name: 'ManageContacts'});
            }).catch(err => { 
                loading.value = false
                toast.value = true
    ...
    return { toast, toastContent, errors, form, saveContact, clearErrors, loading }
    },
}
</script>
